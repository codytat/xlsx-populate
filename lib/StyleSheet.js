...
class StyleSheet {
    ...
    createStyle(sourceId) {
        ...
        // Deduplicate: find or add font
        const fontId = this._findOrAddNode(this._fontsNode, fontNode, this._fontCache);

        // Deduplicate: find or add fill
        const fillId = this._findOrAddNode(this._fillsNode, fillNode, this._fillCache);

        // Deduplicate: find or add border
        const borderId = this._findOrAddNode(this._bordersNode, borderNode, this._borderCache);

        // Get references to the actual nodes (may be existing ones)
        fontNode = this._fontsNode.children[fontId];
        fillNode = this._fillsNode.children[fillId];
        borderNode = this._bordersNode.children[borderId];
    ...
    /**
     * Find an existing node matching the given node, or add it if not found.
     * @param {{}} parentNode - The parent collection node (fonts, fills, borders).
     * @param {{}} node - The node to find or add.
     * @param {Map} cache - The cache map for this node type.
     * @returns {number} The index of the node.
     * @private
     */
    _findOrAddNode(parentNode, node, cache) {
        const key = generateNodeKey(node);
        if (cache.has(key)) {
            return cache.get(key);
        }
        parentNode.children.push(node);
        const index = parentNode.children.length - 1;
        cache.set(key, index);
        return index;
    }
    ...
    _init(node) {
        ...
        // Initialize caches for deduplication
        this._fontCache = new Map();
        this._fillCache = new Map();
        this._borderCache = new Map();

        // Build caches from existing nodes
        this._fontsNode.children.forEach((node, index) => {
            this._fontCache.set(generateNodeKey(node), index);
        });
        this._fillsNode.children.forEach((node, index) => {
            this._fillCache.set(generateNodeKey(node), index);
        });
        this._bordersNode.children.forEach((node, index) => {
            this._borderCache.set(generateNodeKey(node), index);
        });
    }
}
