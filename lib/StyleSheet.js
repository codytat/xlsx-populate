...
/**
 * Generate a unique key for an XML node for deduplication.
 * @param {{}} node - The XML node.
 * @returns {string} A unique string key representing the node.
 * @private
 */
const generateNodeKey = node => {
    if (!node) return '';
    const attrsKey = Object.keys(node.attributes || {})
        .sort()
    ...
    const childrenKey = (node.children || [])
        .map(child => {
            ...
            return `${child.name}{${generateNodeKey(child)}}`;
        })
        .join(',');
    return `[${attrsKey}](${childrenKey})`;
};
...
class StyleSheet {
    ...
    createStyle(sourceId) {
        ...
        // Always add new nodes to the collections (no deduplication at creation time)
        // Deduplication will happen in toXml() when saving
     ...
     * Performs deduplication of fonts, fills, and borders before returning.
     * @returns {{}} The XML form.
     * @ignore
     */
    toXml() {
        // Deduplicate fonts, fills, and borders before saving
        this._deduplicateStyles();
        return this._node;
    }

    /**
     * Deduplicate fonts, fills, and borders, updating cellXfs references.
     * @returns {undefined}
     * @private
     */
    _deduplicateStyles() {
        // Deduplicate fonts
        const fontMap = this._deduplicateCollection(this._fontsNode);
        
        // Deduplicate fills
        const fillMap = this._deduplicateCollection(this._fillsNode);
        
        // Deduplicate borders
        const borderMap = this._deduplicateCollection(this._bordersNode);
        
        // Update cellXfs references
        this._cellXfsNode.children.forEach(xfNode => {
            if (xfNode.attributes.fontId !== undefined) {
                xfNode.attributes.fontId = fontMap[xfNode.attributes.fontId];
            }
            if (xfNode.attributes.fillId !== undefined) {
                xfNode.attributes.fillId = fillMap[xfNode.attributes.fillId];
            }
            if (xfNode.attributes.borderId !== undefined) {
                xfNode.attributes.borderId = borderMap[xfNode.attributes.borderId];
            }
        });
    }

    /**
     * Deduplicate a collection of nodes.
     * @param {{}} collectionNode - The collection node (fonts, fills, or borders).
     * @returns {Object} A mapping from old index to new index.
     * @private
     */
    _deduplicateCollection(collectionNode) {
        const uniqueNodes = [];
        const keyToIndex = new Map();
        const oldToNewIndex = {};

        collectionNode.children.forEach((node, oldIndex) => {
            const key = generateNodeKey(node);
            if (keyToIndex.has(key)) {
                // Duplicate found, map to existing index
                oldToNewIndex[oldIndex] = keyToIndex.get(key);
            } else {
                // New unique node
        ...
        // Replace collection with deduplicated nodes
        collectionNode.children = uniqueNodes;
